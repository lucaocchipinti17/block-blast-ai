...
def run_game(agent: HeuristicAgent, piece_pool: dict, verbose: bool = False) -> dict:
    ...
    while True:
        ...
        for _ in range(3):
            ...
            available = [
                i for i, (p, u) in enumerate(zip(piece_bank, used))
                if not u and board.valid_moves(p)
            ]
            if not available:
                break

            # Agent picks best move
            piece_idx, row, col = agent.best_move(board, piece_bank, used, streak)
